...
const actions = {
  ...
  async fetchNextPageOfPosts({ commit, state }, url) {
    const response = await feedApi.fetchPaginated(getPathAndSearch(url))
  ...
  async getPostData({ commit }, postId) {
    try {
      commit('setPostData', {})
      const response = await postsApi.fetchPost(postId)
  ...
  async getProfilePostList({ commit }, profileSlug) {
    try {
      commit('setProfile', {})
      const response = await feedApi.fetchProfilePosts(profileSlug)
  ...
  async submitPostCommentForm({ state }, payload) {
    ...
    try {
      const response = await postsApi.addComment(postId, commentBody)
  ...
  async fetchNextPageOfSearch({ commit, state }, url) {
    ...
    const response = await searchApi.fetchSearchPage(urlObject.pathname, params)
  ...
  async getTrendPosts({ commit }, trendId) {
    try {
      const response = await feedApi.fetchTrendPosts(trendId)
  ...
  async likePost(context, postId) {
    return actionsApi.likePost(postId)
  },
  async reportPost({ dispatch }, postId) {
    try {
      await actionsApi.reportPost(postId)
      await dispatch(
        'alert/setMessage',
        {
  ...
  async deletePost({ state, commit, dispatch }, postId) {
    try {
      ...
      state.profile.posts_count -= 1
      await actionsApi.deletePost(postId)
      await dispatch(
        'alert/setMessage',
        {
