...
const actions = {
  ...
  async getOrCreateChat({ dispatch }, userSlug) {
    try {
      await chatApi.getOrCreateChat(userSlug)
    } catch (error) {
      dispatch(
        'alert/setMessage',
        {
  ...
  async getChatMessages({ state, commit }) {
    try {
      const response = await chatApi.fetchChatMessages(state.activeConversation)
  ...
  async submitChatForm({ state }, message) {
    try {
      await chatApi.sendChatMessage(state.activeConversation.id, message)
    } catch (error) {
      return Promise.reject(error)
    }
  },
