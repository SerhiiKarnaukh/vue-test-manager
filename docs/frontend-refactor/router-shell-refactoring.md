...
| File | Lines | Role |
|------|------:|------|
| `src/router/index.js` | ~345 | All routes (appsmanager, taberna, social, ai_lab) + guards |
...
|--------|--------|------------------|
| `/`, `/apps_manager/*` | 3 | `apps/core-appsmanager/routes.js` |
| `/taberna/*` | 14 | `apps/taberna/routes.js` (or split per sub-app later) |
| `/social/*` | 14 | `apps/social/routes.js` |
| `/ai-lab/*` | 4 | `apps/ai-lab/routes.js` |
| `/:NotFound(.*)*` | 1 | `shared/router/routes.js` or appsmanager |
...
| 2 | Extract `apps/core-appsmanager/routes.js`; spread in index | `/`, search, 404 |
| 3 | Extract taberna, social, ai-lab route files | Full route table smoke |
...
- [ ] All existing named routes resolve (`name` unchanged).
- [ ] `meta.layout` and `meta.authJWT` behaviour unchanged.
- [ ] `getLoginRoute` still sends taberna → `/taberna/login?redirect=…` and social → `/social/login`.
- [ ] `router/index.js` shim: `export { default } from '@/shared/router'` (or equivalent).
- [ ] `npm run build` and `npm run lint` clean.
- [ ] No route file > ~150 lines (split taberna/social further if needed).
