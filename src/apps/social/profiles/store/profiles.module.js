...
const actions = {
  async getUserData({ commit, dispatch }) {
    try {
      const response = await profileApi.fetchCurrentUser()
  ...
  async sendFriendshipRequest({ dispatch }, userSlug) {
    try {
      const response = await friendshipApi.sendFriendRequest(userSlug)
  ...
  async getCurrentProfileFriendsData({ commit }, userSlug) {
    try {
      const response = await friendshipApi.fetchFriendsData(userSlug)
  ...
  async handleFriendshipRequest({ state, dispatch }, payload) {
    try {
      ...
      await friendshipApi.handleFriendRequest(slug, status)
  ...
  async editProfile({ commit, dispatch, getters }, formData) {
  ...
  async getFriendSuggestions({ commit, rootGetters }) {
    ...
    if (isAuthenticated) {
      try {
        const response = await friendshipApi.fetchFriendSuggestions()
